rmnlIndepMetrop_mod=function(Data,Prior,Mcmc,posttune=TRUE){
#
# revision history:
#   p. rossi 1/05
...
if (posttune==TRUE){
  ...
  tune <- function(beta,y,X,betabar,rootpi){
    llmnl(beta,y,X) -0.5*as.vector(t(rootpi)%*%(beta-betabar))%*%as.vector(t(rootpi)%*%(beta-betabar))      
       #+lndMvn(beta, betabar,rootpi)
    }
  
  mle = optim(beta, tune, X = X, y = y, betabar =betabar, rootpi = rootpi, method = "BFGS", hessian = TRUE, 
...
# 08/21/2014
###################################################################
loopout = rmnlIndepMetrop_rcpp_loop_mod(R,keep,nu,betastar,root,y,X,betabar,rootpi,rooti,oldlimp,oldlpost,nprint);
###################################################################
